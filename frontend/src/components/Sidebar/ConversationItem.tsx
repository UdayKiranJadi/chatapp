...
const ConversationItem: React.FC<Conversation> = ({
    conversationId, friend, unreadCounts, lastMessage}) => {
        ...
        if(lastMessage?.timestamp){
            ...
            const time = createdAt.toLocaleDateString([], {
                hour:"2-digit",
                minute:"2-digit"
            })
        ...
        return <div className={`p-4 border-gray-200 flex items-center space-x-3 cursor-pointer trasition-colors  ${isSelected ? 'bg-blue-100' :'bg-gray-50'}`
        ...
        >
            <div className="relative">
                <img src="https://testingbot.com/free-online-tools/random-avatar/300" alt="user"
                className="size-10 rounded-full object-cover" />
                <div className={`absolute bottom-0 right-0 size-3 rounded-full border-2 border-white ${friend.online ? "bg-green-400" : "bg-gray-400"}`}>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center">
                    <p className={unreadMessages() ? "text-sm text-gray-500 truncate min-h-[20px]" :"text-sm text-sky-500 truncate min-h-[20px]"}>
                        {lastMessage?.content ?? ' '}
